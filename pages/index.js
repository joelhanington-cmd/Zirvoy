...
function BookingScreen({trip,onBack,onDone,onSummary,homeAirport}){
  ...
  const allSteps=["Dates","Flights","Hotels"];
  ...
  // Hotel finder
  const[hotelStyle,setHotelStyle]=useState("Mid-range comfort");
  const[hotelPriority,setHotelPriority]=useState("City-centre location");
    ...
    <CyclingHeader images={(trip.storySlides||[]).map(s=>s.imageUrl).filter(Boolean)} minH="220px">
      <div style={{padding:"2.5rem 1.5rem 2rem"}}>
        ...
        <div style={{display:"flex",gap:6,marginTop:"1.25rem"}}>
          {allSteps.map((s,i)=>(<div key={i} style={{flex:1,height:3,borderRadius:2,background:i===0?"rgba(196,98,45,0.8)":"rgba(242,232,217,0.15)"}}/>))}
        </div>
        <p style={{fontSize:"0.72rem",color:"rgba(242,232,217,0.4)",margin:"0.5rem 0 0",fontWeight:300}}>Step 1 of 3</p>
      </div>
    </CyclingHeader>
  ...
  return(<div style={{minHeight:"100vh",background:C.sandLight,fontFamily:"'DM Sans',sans-serif",paddingBottom:80}}>
    ...
    <CyclingHeader images={(trip.storySlides||[]).map(s=>s.imageUrl).filter(Boolean)} minH="220px">
      <div style={{padding:"2.5rem 1.5rem 2rem"}}>
        ...
        <p style={{fontSize:"0.82rem",color:"rgba(242,232,217,0.5)",margin:0,fontWeight:300}}>{step<steps.length?`Step ${step+2} of 3`:"Trip confirmed ✓"}</p>
        <div style={{display:"flex",gap:6,marginTop:"1.25rem"}}>
          {allSteps.map((s,i)=>(<div key={i} style={{flex:1,height:3,borderRadius:2,background:i===0||i-1<=step?"rgba(196,98,45,0.8)":"rgba(242,232,217,0.15)",transition:"background 0.3s"}}/>))}
        </div>
      </div>
    </CyclingHeader>
    ...
    <div style={{padding:"1.5rem",maxWidth:640,margin:"0 auto"}}>
      ...
      <div>
        ...
        <button onClick={onSummary||onDone}
          style={{width:"100%",padding:"1rem",background:C.terracotta,color:C.white,border:"none",borderRadius:12,fontSize:"0.95rem",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",marginBottom:"1.25rem"}}>
          View & plan my trip →
        </button>
...
function AccountScreen({profile,onSignOut,destImages=[],onUpdateProfile}){
  ...
  const handleSave=async()=>{
    setSaving(true);
    const ok=await onUpdateProfile(form);
    setSaving(false);
    if(ok){setSavedMsg(true);setEditing(false);setTimeout(()=>setSavedMsg(false),2500);}
  };
  return(
    <div style={{minHeight:"100vh",background:C.sandLight,fontFamily:"'DM Sans',sans-serif",paddingBottom:80}}>
      <CyclingHeader images={destImages} minH="200px">
        <div style={{padding:"3rem 1.5rem 2.5rem"}}>
          <ZirvoyLogo light/>
          <div style={{marginTop:"1.5rem",display:"flex",alignItems:"center",gap:"1rem"}}>
            <div style={{width:52,height:52,borderRadius:"50%",background:"rgba(196,98,45,0.2)",border:`1.5px solid ${C.terracotta}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.4rem",fontWeight:600,color:C.sand}}>{(profile?.first_name?.[0]||"Z").toUpperCase()}</span>
            </div>
            <div>
              <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.4rem",fontWeight:600,color:C.sand,margin:0}}>{profile?.first_name} {profile?.last_name}</p>
      ...
      <div style={{padding:"1.5rem"}}>
        {saved&&<div style={{background:"#e8f5e9",border:"1px solid #81c784",borderRadius:10,padding:"0.65rem 1rem",marginBottom:"1rem",fontSize:"0.85rem",color:"#2e7d32",fontWeight:500}}>Profile updated</div>}
        <div style={{background:C.white,borderRadius:18,border:`1px solid ${C.border}`,padding:"1.25rem",marginBottom:"1rem",boxShadow:"0 2px 12px rgba(28,20,16,0.05)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem"}}>
            <p style={{fontSize:"0.68rem",fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:"0.12em",margin:0}}>Your Details</p>
            {!editing&&<button onClick={()=>setEditing(true)} style={{background:"transparent",border:`1px solid ${C.border}`,borderRadius:8,padding:"0.3rem 0.75rem",fontSize:"0.78rem",color:C.espresso,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:500}}>Edit</button>}
          </div>
          {editing?(
            <>
              ...
              <div style={{display:"flex",gap:"0.5rem",marginTop:"0.5rem"}}>
                <button onClick={handleSave} disabled={saving} style={{flex:1,padding:"0.85rem",background:C.terracotta,color:C.white,border:"none",borderRadius:10,fontSize:"0.88rem",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{saving?"Saving…":"Save changes"}</button>
                <button onClick={()=>setEditing(false)} style={{padding:"0.85rem 1rem",background:"transparent",color:C.muted,border:`1px solid ${C.border}`,borderRadius:10,fontSize:"0.88rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
              </div>
            </>
          ):(
            <>
              {[["Home Airport",profile?.home_airport||"Not set"],["Nationality",profile?.nationality||"Not set"],["Date of Birth",profile?.date_of_birth||"Not set"]].map(([label,val])=>(
                <div key={label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingBottom:"0.85rem",marginBottom:"0.85rem",borderBottom:`1px solid ${C.parchment}`}}>
                  ...
                  <span style={{fontSize:"0.85rem",color:C.espresso,fontWeight:500}}>{val}</span>
                </div>
              ))}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:"0.85rem",color:C.muted}}>Passport</span>
                <span style={{fontSize:"0.85rem",color:profile?.passport_number?C.espresso:C.muted,fontWeight:profile?.passport_number?500:300}}>{profile?.passport_number?"••••••••••":"Not added"}</span>
              </div>
            </>
          )}
        </div>
...
function LoginScreen({onSuccess,onCreateAccount}){
  ...
  const handleLogin=async()=>{setLoading(true);setError(null);try{const{data,error:ae}=await supabase.auth.signInWithPassword({email:form.email,password:form.password});if(ae)throw ae;onSuccess(data.user);}catch(e){setError("Incorrect email or password — please try again.");}finally{setLoading(false);};};
  const handleForgot=async()=>{if(!form.email)return;setForgotLoading(true);await supabase.auth.resetPasswordForEmail(form.email,{redirectTo:"https://zirvoy.com"});setForgotSent(true);setForgotLoading(false);};
  return(
    <div style={{minHeight:"100vh",background:C.sandLight,fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{background:C.espresso,padding:"2rem 1.5rem 2.5rem",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative"}}><ZirvoyLogo light/>
          <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"2rem",fontWeight:600,color:C.sand,margin:"1.25rem 0 0.35rem"}}>{showForgot?"Reset password":"Welcome back"}</h2>
          <p style={{fontSize:"0.85rem",color:"rgba(242,232,217,0.5)",margin:0,fontWeight:300}}>{showForgot?"We'll send a reset link to your email":"Log in to access your trips"}</p>
        </div>
      </div>
      <div style={{padding:"1.75rem 1.5rem",maxWidth:480,margin:"0 auto"}}>
        ...
        {showForgot?(
          forgotSent?(
            <div style={{background:C.parchment,borderRadius:12,padding:"1.25rem",marginBottom:"1rem",textAlign:"center"}}>
              <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.3rem",color:C.espresso,margin:"0 0 0.4rem"}}>Check your inbox</p>
              <p style={{fontSize:"0.85rem",color:C.muted,margin:"0 0 1rem",fontWeight:300}}>We've sent a reset link to {form.email}</p>
              <button onClick={()=>{setShowForgot(false);setForgotSent(false);}} style={{background:"transparent",border:"none",color:C.terracotta,cursor:"pointer",fontSize:"0.85rem",fontWeight:500,fontFamily:"'DM Sans',sans-serif"}}>← Back to login</button>
            </div>
          ):(
            <>
              <Input label="Email" type="email" value={form.email} onChange={set("email")} placeholder="john@example.com"/>
              <Btn onClick={handleForgot} disabled={!form.email||forgotLoading}>{forgotLoading?"Sending…":"Send reset link →"}</Btn>
              <button onClick={()=>setShowForgot(false)} style={{width:"100%",marginTop:"0.75rem",background:"transparent",border:"none",color:C.muted,cursor:"pointer",fontSize:"0.85rem",fontFamily:"'DM Sans',sans-serif"}}>← Back to login</button>
            </>
          )
        ):(
          <>
            ...
            <p style={{textAlign:"center",fontSize:"0.85rem",color:C.muted,marginTop:"1rem"}}>
              <span onClick={()=>setShowForgot(true)} style={{color:C.terracotta,cursor:"pointer",fontWeight:500}}>Forgot password?</span>
            </p>
            <p style={{textAlign:"center",fontSize:"0.85rem",color:C.muted,marginTop:"0.5rem"}}>Don't have an account?{" "}<span onClick={onCreateAccount} style={{color:C.terracotta,cursor:"pointer",fontWeight:500}}>Create one</span></p>
          </>
        )}
      </div>
    </div>
...
function TripStoryScreen({trip,onSkip,onBook}){
  ...
  return(<div style={{position:"fixed",inset:0,background:C.espresso,fontFamily:"'DM Sans',sans-serif",userSelect:"none"}} onMouseDown={()=>setPaused(true)} onMouseUp={()=>setPaused(false)} onTouchStart={()=>setPaused(true)} onTouchEnd={()=>setPaused(false)}>
    ...
    <div style={{position:"absolute",top:"1.5rem",left:0,right:0,padding:"0 1rem",display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:10,marginTop:"2.5rem"}}>
      ...
      <button onClick={onSkip} style={{background:"rgba(28,20,16,0.4)",backdropFilter:"blur(8px)",border:"1px solid rgba(242,232,217,0.2)",color:C.sand,padding:"0.35rem 0.9rem",borderRadius:20,fontSize:"0.75rem",fontWeight:500,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Exit →</button>
    </div>
...
function DecideModal({onClose,onGenerate,profile}){
    ...
    <div style={{minHeight:"100vh",background:C.sandLight,fontFamily:"'DM Sans',sans-serif",paddingBottom:80}}>
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:"1.5rem 1.5rem"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.65rem",marginBottom:"1.25rem"}}>
          {suggestions.map((s,i)=>(
            <button key={i} onClick={()=>!loading&&onGenerate(s.text)} disabled={loading}
              style={{padding:"0.9rem",borderRadius:12,border:`1px solid ${C.border}`,background:C.white,cursor:loading?"default":"pointer",fontFamily:"'DM Sans',sans-serif",textAlign:"left",boxShadow:"0 1px 6px rgba(28,20,16,0.04)"}}>
        ...
        <button onClick={onDecide} disabled={loading}
          style={{width:"100%",padding:"1.1rem",background:C.terracotta,color:C.white,border:"none",borderRadius:14,fontSize:"0.98rem",fontWeight:600,cursor:loading?"default":"pointer",fontFamily:"'DM Sans',sans-serif",marginBottom:"1.25rem",opacity:loading?0.5:1,display:"flex",alignItems:"center",justifyContent:"center",gap:"0.5rem",boxShadow:"0 4px 20px rgba(196,98,45,0.28)"}}>
          ✨ Help me plan my trip
        </button>

        {/* Freeform input — for when you know exactly what you want */}
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:"0.85rem"}}>
          <div style={{flex:1,height:1,background:C.border}}/>
          <span style={{fontSize:"0.65rem",fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:"0.14em",whiteSpace:"nowrap"}}>or describe it yourself</span>
          <div style={{flex:1,height:1,background:C.border}}/>
        </div>
        <div style={{background:C.white,borderRadius:18,padding:"1.1rem",marginBottom:"1.5rem",boxShadow:"0 2px 12px rgba(28,20,16,0.06)",border:`1px solid ${C.border}`}}>
          <textarea value={input} onChange={e=>setInput(e.target.value)} onFocus={()=>setFocused(true)} onBlur={()=>setFocused(false)}
            ...
            rows={3} style={{width:"100%",background:C.sandLight,border:`1.5px solid ${focused?C.terracotta:C.border}`,borderRadius:10,padding:"0.85rem",fontSize:"0.9rem",color:C.ink,resize:"none",outline:"none",lineHeight:1.65,fontFamily:"'DM Sans',sans-serif",transition:"border-color 0.2s",boxSizing:"border-box"}}/>
          <button onClick={()=>input.trim()&&!loading&&onGenerate(input.trim())} disabled={!input.trim()||loading}
            style={{width:"100%",marginTop:"0.65rem",padding:"0.9rem",background:input.trim()&&!loading?C.espresso:C.parchment,color:input.trim()&&!loading?C.sand:C.muted,border:"none",borderRadius:10,fontSize:"0.9rem",fontWeight:600,cursor:input.trim()&&!loading?"pointer":"default",fontFamily:"'DM Sans',sans-serif",transition:"background 0.25s"}}>
        ...
        {trips.length>0&&(
          <div style={{marginBottom:"1rem"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"0.75rem"}}>
              <p style={{fontSize:"0.68rem",fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:"0.12em",margin:0}}>Recent Trips</p>
              {onViewTrips&&<button onClick={onViewTrips} style={{background:"transparent",border:"none",color:C.terracotta,fontSize:"0.78rem",fontWeight:500,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",padding:0}}>See all →</button>}
            </div>
...
function ResultsScreen({trip:initialTrip,onNewTrip,onTryAgain,onLetsBook,onSaveTrip,isSaved,onShowStory,onPlanTrip,tripId,user,onBack}){
  const[trip,setTrip]=useState(initialTrip);
  const[activeDay,setActiveDay]=useState(0);
  const[imgLoaded,setImgLoaded]=useState(false);
  const[saving,setSaving]=useState(false);
  const[saved,setSaved]=useState(isSaved);
  // Keep saved in sync with parent tripSaved (e.g. after auto-save on login)
  useEffect(()=>{setSaved(isSaved);},[isSaved]);
  const[dayImages,setDayImages]=useState({});
  const[copied,setCopied]=useState(false);
  const[tipLoading,setTipLoading]=useState(false);
  const[tipFresh,setTipFresh]=useState(false);
  const[bannerDismissed,setBannerDismissed]=useState(()=>{try{return sessionStorage.getItem("zirvoy_banner_dismissed")==="1";}catch{return false;}});
  const dismissBanner=()=>{setBannerDismissed(true);try{sessionStorage.setItem("zirvoy_banner_dismissed","1");}catch{}};
  ...
  const copyShare=()=>{
    const val=shareUrl||shareMsg;
    const fallback=()=>{
      try{const el=document.createElement("textarea");el.value=val;el.style.position="fixed";el.style.opacity="0";document.body.appendChild(el);el.select();document.execCommand("copy");document.body.removeChild(el);setCopied(true);setTimeout(()=>setCopied(false),2500);}catch(e){}
    };
    if(navigator.clipboard?.writeText){navigator.clipboard.writeText(val).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2500);}).catch(fallback);}else{fallback();}
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:C.sandLight,fontFamily:"'DM Sans',sans-serif",paddingBottom:80}}>
      ...
      <div style={{position:"relative",height:"min(340px,50vw)",minHeight:260,overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:"1.25rem",left:"clamp(1.25rem,5vw,2rem)",display:"flex",alignItems:"center",gap:"0.6rem"}}>
          {onBack&&<button onClick={onBack} style={{background:"rgba(28,20,16,0.45)",backdropFilter:"blur(8px)",border:"1px solid rgba(242,232,217,0.22)",color:C.sand,padding:"0.28rem 0.65rem",borderRadius:20,fontSize:"0.72rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>←</button>}
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:"1.25rem clamp(1.25rem,5vw,2rem) 3rem"}}>
        ...
        {/* Guest save banner — dismissable */}
        {!user&&!bannerDismissed&&(
          <div style={{background:C.parchment,border:`1px solid ${C.border}`,borderRadius:14,padding:"0.8rem 1rem",marginBottom:"1rem",display:"flex",alignItems:"center",gap:"0.75rem"}}>
            <div style={{flex:1}}>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"0.82rem",fontWeight:600,color:C.espresso,margin:"0 0 0.1rem"}}>Don't lose this trip</p>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"0.75rem",color:C.muted,margin:0,fontWeight:300}}>Sign up free to save it permanently</p>
            </div>
            <button onClick={onSaveTrip} style={{flexShrink:0,padding:"0.5rem 0.9rem",background:C.terracotta,color:C.white,border:"none",borderRadius:9,fontSize:"0.8rem",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>Save free →</button>
            <button onClick={dismissBanner} style={{flexShrink:0,background:"transparent",border:"none",color:C.muted,cursor:"pointer",fontSize:"1rem",lineHeight:1,padding:"0.25rem",fontFamily:"'DM Sans',sans-serif"}}>×</button>
          </div>
        )}

        {/* Story teaser — prominent entry point */}
        {trip.storySlides?.length>0&&onShowStory&&(
          <button onClick={onShowStory} style={{width:"100%",padding:"0.85rem 1.1rem",background:C.espresso,border:"none",borderRadius:14,marginBottom:"1rem",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
            <div style={{display:"flex",alignItems:"center",gap:"0.65rem"}}>
              <span style={{fontSize:"1.1rem"}}>▶</span>
              <div style={{textAlign:"left"}}>
                <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1rem",fontWeight:600,color:C.sand,margin:0}}>Watch your trip story</p>
                <p style={{fontSize:"0.72rem",color:"rgba(242,232,217,0.5)",margin:0,fontWeight:300}}>Photo highlight reel · {trip.storySlides.length} slides</p>
              </div>
            </div>
            <span style={{color:C.terracotta,fontSize:"1.3rem"}}>→</span>
          </button>
        )}

        {/* Primary action block — booked vs unbooked */}
        {trip.isBooked?(
          <div style={{background:C.espresso,borderRadius:14,padding:"1rem 1.1rem",marginBottom:"1rem",display:"flex",alignItems:"center",gap:"0.75rem"}}>
            <span style={{fontSize:"1.3rem"}}>✈</span>
            <div style={{flex:1}}>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"0.85rem",fontWeight:600,color:C.sand,margin:0}}>Trip booked</p>
              {trip.departDate&&<p style={{fontSize:"0.72rem",color:"rgba(242,232,217,0.55)",margin:"0.1rem 0 0",fontWeight:300}}>{trip.departDate}</p>}
            </div>
            <button onClick={onPlanTrip} style={{padding:"0.55rem 1rem",background:C.terracotta,color:C.white,border:"none",borderRadius:9,fontSize:"0.8rem",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>View plan →</button>
          </div>
        ):(
          <div style={{marginBottom:"0.75rem"}}>
            <button onClick={onLetsBook} style={{width:"100%",padding:"1.05rem",background:C.terracotta,color:C.white,border:"none",borderRadius:12,fontSize:"0.95rem",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",marginBottom:"0.45rem"}}>Find flights & hotels →</button>
            <button onClick={onPlanTrip} style={{width:"100%",padding:"0.85rem",background:C.white,color:C.espresso,border:`1.5px solid ${C.border}`,borderRadius:12,fontSize:"0.88rem",fontWeight:500,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Plan in detail →</button>
          </div>
        )}

        {/* Weather — moved up, decision-critical */}
        {trip.weather&&(
          <div style={{background:C.white,borderRadius:18,border:`1px solid ${C.border}`,padding:"1.1rem 1.35rem",marginBottom:"1rem",boxShadow:"0 2px 16px rgba(28,20,16,0.05)"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:"0.75rem"}}><div style={{width:3,height:14,background:C.terracotta,borderRadius:2}}/><p style={{fontSize:"0.68rem",fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:"0.12em",margin:0}}>🌤 When to go</p></div>
            <p style={{margin:"0 0 0.85rem",fontSize:"0.88rem",color:C.ink,lineHeight:1.65,fontWeight:300}}>{trip.weather.insight}</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.5rem"}}>
              {[["✅ Best time",trip.weather.bestMonths],["🌡 Temperature",trip.weather.tempRange],["📈 Peak season",trip.weather.peakSeason],trip.weather.avoidMonths&&["⚠️ Avoid",trip.weather.avoidMonths]].filter(Boolean).map(([label,val])=>(<div key={label} style={{background:C.sandLight,borderRadius:10,padding:"0.65rem 0.75rem"}}><div style={{fontSize:"0.6rem",fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:"0.25rem"}}>{label}</div><div style={{fontSize:"0.8rem",fontWeight:500,color:C.espresso}}>{val}</div></div>))}
            </div>
          </div>
        )}
        ...
        <div style={{background:C.white,borderRadius:18,border:`1px solid ${C.border}`,padding:"1.35rem",marginBottom:"1rem",boxShadow:"0 2px 16px rgba(28,20,16,0.05)"}}>
          ...
          <div style={{background:C.sandLight,borderRadius:8,padding:"0.6rem 0.85rem",marginBottom:"1rem",fontSize:"0.72rem",color:C.muted,lineHeight:1.5}}>Estimates based on typical costs — always check live prices before booking.</div>
        ...
        <div style={{background:tipFresh?"#2a1a0e":C.espresso,borderRadius:18,padding:"1.4rem 1.5rem",marginBottom:"1rem",position:"relative",overflow:"hidden",transition:"background 0.4s ease"}}>
          ...
          <p key={trip.tip} style={{margin:0,fontFamily:"'Cormorant Garamond',serif",fontSize:"1.1rem",color:C.sand,lineHeight:1.8,fontStyle:"italic"}}>{trip.tip}</p>
        </div>
        ...
        {saved&&(
          <div style={{background:C.parchment,borderRadius:12,padding:"0.85rem 1rem",marginBottom:"0.5rem",display:"flex",alignItems:"center",gap:"0.5rem"}}>
            <span style={{color:C.terracotta}}>✓</span>
            <span style={{fontSize:"0.85rem",color:C.espresso,fontWeight:500,fontFamily:"'DM Sans',sans-serif"}}>Trip saved to your account</span>
          </div>
        )}
...
export default function Home(){
  ...
  const handleUpdateProfile=async(updates)=>{if(!user)return false;const{error}=await supabase.from("profiles").update(updates).eq("id",user.id);if(!error){setProfile(p=>({...p,...updates}));return true;}return false;};
  ...
  const handleSummaryClick=(t)=>{setTrip(t.trip_data);setTripSaved(true);setShowStory(false);setShowBooking(false);setShowSummary(true);setScreen("results");};
  ...
  return(<>
    <Head>
      ...
      <meta name="description" content="Tell Zirvoy where you want to go. Get a full AI trip plan — flights, hotel, day-by-day itinerary — in seconds. Free to use."/>
      <meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"/>
      <meta property="og:type" content="website"/>
      <meta property="og:site_name" content="Zirvoy"/>
      <meta property="og:title" content="Zirvoy — AI Travel Planner"/>
      <meta property="og:description" content="Tell Zirvoy where you want to go. Get a full AI trip plan — flights, hotel, day-by-day itinerary — in seconds. Free to use."/>
      <meta property="og:image" content="https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1200&q=80"/>
      <meta property="og:image:width" content="1200"/>
      <meta property="og:image:height" content="630"/>
      <meta property="og:url" content="https://zirvoy.com"/>
      <meta property="og:locale" content="en_GB"/>
      <meta name="twitter:card" content="summary_large_image"/>
      <meta name="twitter:title" content="Zirvoy — AI Travel Planner"/>
      <meta name="twitter:description" content="Full AI trip plan — flights, hotel, itinerary — in seconds. Free to use."/>
    ...
    {(user||authScreen==="app")&&authScreen!=="signup"&&authScreen!=="login"&&(<>
      ...
      {!(showStory&&screen==="results")&&!showBooking&&!showSummary&&(<>
        ...
        {activeTab==="account"&&(user
          ?<AccountScreen profile={profile} onSignOut={handleSignOut} destImages={destImages} onUpdateProfile={handleUpdateProfile}/>
